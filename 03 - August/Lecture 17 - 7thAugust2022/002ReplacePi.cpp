...
void replacePi(char* inp, int i) {
	...
	if(inp[i] == 'p' and inp[i+1] == 'i') {
		...
		// shift all the characters starting from (i+2)th idx two steps to the right
		int j = i+2;
		while(inp[j] != '\0') {
			j++;
		}
		...
		// ask your friend to replace "pi"'s with "3.14" in inp[i+4...n-1]
		replacePi(inp, i+4);

	} else {
		// ask your friend to replace "pi"'s with "3.14" in inp[i+1...n-1]
		replacePi(inp, i+1);
	}
